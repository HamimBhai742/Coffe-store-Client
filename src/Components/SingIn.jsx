...
const SingIn = () => {
  const { handelLoginUser } = useContext(AuthContext);
  const [showPass, setShowPass] = useState(false);
  const handelSingInBtn = (e) => {
    e.preventDefault();
    const form = e.target;
    ...
    handelLoginUser(email, password)
      .then((result) => {
        ...
        axios
          .patch("https://coffe-store-server-cyan.vercel.app/user", user)
          .then((data) => {
  ...
  return (
    <div className="flex flex-col max-w-md p-6 mx-auto mt-10 rounded-md sm:p-10 bg-[#F4F3F0] font-raleway">
      ...
      <form onSubmit={handelSingInBtn} className="space-y-12">
      ...
      <div className="flex items-center pt-4 space-x-1">
        ...
        <p className="px-3 text-lg text-gray-600">Login with social accounts</p>
        <div className="flex-1 h-px sm:w-16 dark:bg-gray-300"></div>
      </div>
